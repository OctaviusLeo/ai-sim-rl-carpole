...
def test_evaluate_model_basic():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        returns, success_rate = evaluate_model(
            model, env_name="CartPole-v1", n_episodes=10, seed=123
        )
...
def test_multi_seed_evaluation():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        model, _ = train_ppo(
            env_name="CartPole-v1", total_timesteps=2000, seed=42, output_dir=tmpdir, verbose=0
        )

        # Run multi-seed evaluation
        results = multi_seed_evaluation(
            model, env_name="CartPole-v1", episodes_per_seed=5, num_seeds=3, base_seed=100
        )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for n_episodes in [1, 5, 10]:
            returns, _ = evaluate_model(
                model, env_name="CartPole-v1", n_episodes=n_episodes, seed=42
            )

            assert len(returns) == n_episodes, f"Expected {n_episodes} returns, got {len(returns)}"
...
def test_success_rate_calculation():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        # Train a model
        model, _ = train_ppo(
            env_name="CartPole-v1", total_timesteps=5000, seed=42, output_dir=tmpdir, verbose=0
        )

        returns, success_rate = evaluate_model(
            model, env_name="CartPole-v1", n_episodes=20, seed=123, success_threshold=500.0
        )
        ...
        assert (
            abs(success_rate - expected_success_rate) < 0.01
