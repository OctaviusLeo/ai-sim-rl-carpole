...
def test_seed_determinism():
    ...
    with tempfile.TemporaryDirectory() as tmpdir1, \
        ...
        assert metrics1["total_episodes"] == metrics2["total_episodes"], \
            "Episode counts differ between runs with same seed"
        
        # Training curves should be very similar (allowing small numerical error)
        assert abs(metrics1["final_mean_return"] - metrics2["final_mean_return"]) < 1.0, \
...
def test_different_seeds_produce_different_results():
    ...
    with tempfile.TemporaryDirectory() as tmpdir1, \
        ...
        assert returns_differ or episodes_differ, \
...
def test_evaluation_determinism():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        np.testing.assert_array_equal(
            returns1, returns2,
            err_msg="Evaluation returns differ with same seed"
        )
